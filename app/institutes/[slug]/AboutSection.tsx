...
export default function AboutPage({ institute }: AboutSectionProps) {
  const about = institute?.about || {};

  const slides =
    about?.heroSlides?.length > 0
      ? about.heroSlides
      : [
          {
            id: 1,
            image: institute?.aboutImage || institute?.heroImage || "/placeholder.jpg",
            tag: "ABOUT US",
            title: `About ${institute?.title || "Institute"}`,
          },
        ];
  ...
  const aboutTitle =
    about?.sectionTitle || `About ${institute?.title || "Institute"}`;

  const aboutParagraphs =
    about?.paragraphs?.length > 0
      ? about.paragraphs
      : [institute?.description || "No content available."];

  const academicTag = about?.academicTag || "Academic Excellence";
  const academicTitle =
    about?.academicTitle ||
    `The ${institute?.title || "Institute"} Academic Standard`;

  const academicParagraphs =
    about?.academicParagraphs?.length > 0
      ? about.academicParagraphs
      : [];

  const features = about?.features?.length > 0 ? about.features : [];

  const journeyTitle =
    about?.journeyTitle || `The ${institute?.title || "Institute"} Journey`;

  const timeline = about?.timeline?.length > 0 ? about.timeline : [];
    ...
    <main className="w-full bg-white">
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <div className="space-y-5">
              {academicParagraphs.map((paragraph: string, index: number) => (
                <p
                  key={index}
      ...
      <section className="relative bg-[#f8fafc] py-24 px-6 md:px-16 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <div className={`grid ${gridColsClass} gap-12 md:gap-8`}>
            {timeline.map(
              (
